    ...
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        @if ($permissions['create'])
            <div class="d-flex gap-2">
                <a href="{{ route('anggaran.rencana-kerja-direktorat.import') }}" class="btn btn-success"><i
                        class="bi bi-file-earmark-excel me-1"></i> Import Excel</a>
                <a href="{{ route('anggaran.rencana-kerja-direktorat.create') }}" class="btn btn-primary"><i
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                                    ...
                                    @if ($item->file_name)
                                        @if ($item->userHasFileAccess(auth()->id()))
                                            <div class="btn-group btn-group-sm">
                                                <button
                                                    onclick="previewFile('{{ route('anggaran.rencana-kerja-direktorat.preview', $item->id) }}', '{{ $item->file_name }}')"
                                                    class="btn btn-outline-primary" title="Preview"><i
                                                        class="bi bi-eye"></i></button>
                                                <a href="{{ route('anggaran.rencana-kerja-direktorat.download', $item->id) }}"
                                        ...
                                        @else
                                            <button type="button" class="btn btn-sm btn-outline-warning"
                                                ...
                                                data-title="{{ $item->uraian ?? $item->file_name }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $permissions['download'] ? 6 : 5 }}" class="text-center py-4 text-muted">
                                    Belum
                                    ada data</td>
                            </tr>
                        @endforelse
