...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Penomoran</h1>
    </div>
    <div class="card">
        <div class="card-body">
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if($permissions['preview'] ?? false)
                                            <button
                                        onclick="previewFile('{{ route('hukum-kepatuhan.penomoran.preview', $record->id) }}', '{{ $record->file_name }}')"
                                        class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i> Preview</button>
                                            @endif
                                    <a href="{{ route('hukum-kepatuhan.penomoran.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                ...
                @else
                    <a href="{{ route('hukum-kepatuhan.penomoran.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('hukum-kepatuhan.penomoran.edit', $record->id) }}" class="btn btn-warning"><i
                                class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('hukum-kepatuhan.penomoran.destroy', $record->id) }}" method="POST"
