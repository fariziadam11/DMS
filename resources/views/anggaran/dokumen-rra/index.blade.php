    ...
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        @if ($permissions['create'])
            <div class="d-flex gap-2">
                <a href="{{ route('anggaran.dokumen-rra.import') }}" class="btn btn-success"><i
                        class="bi bi-file-earmark-excel me-1"></i> Import Excel</a>
                <a href="{{ route('anggaran.dokumen-rra.create') }}" class="btn btn-primary"><i
                                    ...
                                    @if ($item->file_name)
                                        @if ($item->userHasFileAccess(auth()->id()))
                                            <div class="btn-group btn-group-sm">
                                                <button
                                                    onclick="previewFile('{{ route('anggaran.dokumen-rra.preview', $item->id) }}', '{{ $item->file_name }}')"
                                                    class="btn btn-outline-primary" title="Preview"><i
                                                        class="bi bi-eye"></i> Preview</button>
                                                <a href="{{ route('anggaran.dokumen-rra.download', $item->id) }}"
