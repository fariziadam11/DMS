...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Rencana Kerja Tahunan</h1>
    </div>
    <div class="card">
        <div class="card-body">
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if ($permissions['preview'] ?? false)
                                        <button
                                            onclick="previewFile('{{ route('anggaran.rencana-kerja-tahunan.preview', $record->id) }}', '{{ $record->file_name }}')"
                                            class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i>
                                            Preview</button>
                                    @endif
                                    <a href="{{ route('anggaran.rencana-kerja-tahunan.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                ...
                @else
                    <a href="{{ route('anggaran.rencana-kerja-tahunan.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('anggaran.rencana-kerja-tahunan.edit', $record->id) }}"
                            class="btn btn-warning"><i class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('anggaran.rencana-kerja-tahunan.destroy', $record->id) }}" method="POST"
    ...
    @include('components.document-tags', [
        ...
        'allTags' => $allTags,
        'module' => 'anggaran',
        'submodule' => 'rencana-kerja-tahunan',
