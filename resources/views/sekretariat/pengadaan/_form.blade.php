<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="kategori" class="form-select @error('kategori') is-invalid @enderror">
            ...
            <option value="Justifikasi" {{ old('kategori', $item->kategori ?? '') == 'Justifikasi' ? 'selected' : '' }}>
                Justifikasi</option>
            <option value="Kontrak" {{ old('kategori', $item->kategori ?? '') == 'Kontrak' ? 'selected' : '' }}>Kontrak
            </option>
            <option value="SPMK" {{ old('kategori', $item->kategori ?? '') == 'SPMK' ? 'selected' : '' }}>SPMK
            </option>
            <option value="Berita Acara"
                {{ old('kategori', $item->kategori ?? '') == 'Berita Acara' ? 'selected' : '' }}>Berita Acara</option>
        </select>
        @error('kategori')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="text" name="masa_akhir" class="form-control @error('masa_akhir') is-invalid @enderror"
            value="{{ old('masa_akhir', $item->masa_akhir ?? '') }}">
        @error('masa_akhir')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="col-md-6 mb-3">
        ...
        <select name="sifat_dokumen" class="form-select">
            ...
            <option value="Rahasia"
                {{ old('sifat_dokumen', $item->sifat_dokumen ?? '') == 'Rahasia' ? 'selected' : '' }}>Rahasia</option>
        </select>
    </div>
    <div class="col-md-6 mb-3">
