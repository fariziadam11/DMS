...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('akuntansi.laporan-audit-keuangan.index') }}">Laporan Audit Keuangan</a>
    </li>
    <li class="breadcrumb-item active">Detail</li>
@endsection
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Laporan Audit Keuangan</h1>
    </div>
    <div class="card">
        <div class="card-body">
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if ($permissions['preview'] ?? false)
                                        <button
                                            onclick="previewFile('{{ route('akuntansi.laporan-audit-keuangan.preview', $record->id) }}', '{{ $record->file_name }}')"
                                            class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i>
                                            Preview</button>
                                    @endif
                                    <a href="{{ route('akuntansi.laporan-audit-keuangan.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                ...
                @else
                    <a href="{{ route('akuntansi.laporan-audit-keuangan.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('akuntansi.laporan-audit-keuangan.edit', $record->id) }}"
                            class="btn btn-warning"><i class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('akuntansi.laporan-audit-keuangan.destroy', $record->id) }}" method="POST"
    ...
    @include('components.document-tags', [
        ...
        'allTags' => $allTags,
        'module' => 'akuntansi',
        'submodule' => 'laporan-audit-keuangan',
