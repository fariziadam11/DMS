    ...
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        @if ($permissions['create'])
            <div class="d-flex gap-2">
                <a href="{{ route('keuangan.surat-bayar.import') }}" class="btn btn-success"><i
                        class="bi bi-file-earmark-excel me-1"></i> Import Excel</a>
                <a href="{{ route('keuangan.surat-bayar.create') }}" class="btn btn-primary"><i
                    ...
                    <tbody>
                        @forelse($items as $i => $item)
                            <tr>
                                ...
                                <td>{{ $item->tanggal_surat_bayar ? date('d/m/Y', strtotime($item->tanggal_surat_bayar)) : '-' }}
                                </td>
                                    ...
                                    @if ($item->file_name)
                                        @if ($item->userHasFileAccess(auth()->id()))
                                            <div class="btn-group btn-group-sm">
                                                <button
                                                    onclick="previewFile('{{ route('keuangan.surat-bayar.preview', $item->id) }}', '{{ $item->file_name }}')"
                                                    class="btn btn-outline-primary" title="Preview"><i
                                                        class="bi bi-eye"></i> Preview</button>
                                                <a href="{{ route('keuangan.surat-bayar.download', $item->id) }}"
                                        ...
                                        @else
                                            <button type="button" class="btn btn-sm btn-outline-warning"
                                                ...
                                                data-title="{{ $item->nomor_surat_bayar ?? $item->file_name }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $permissions['download'] ? 5 : 4 }}" class="text-center py-4 text-muted">
                                    Belum
                                    ada data</td>
                            </tr>
                        @endforelse
