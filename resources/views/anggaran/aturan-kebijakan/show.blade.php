...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Aturan Kebijakan Anggaran</h1>
    </div>
    <div class="card">
        <div class="card-body">
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if ($permissions['preview'] ?? false)
                                        <button
                                            onclick="previewFile('{{ route('anggaran.aturan-kebijakan.preview', $record->id) }}', '{{ $record->file_name }}')"
                                            class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i>
                                            Preview</button>
                                    @endif
                                    <a href="{{ route('anggaran.aturan-kebijakan.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                ...
                @else
                    <a href="{{ route('anggaran.aturan-kebijakan.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('anggaran.aturan-kebijakan.edit', $record->id) }}" class="btn btn-warning"><i
                                class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('anggaran.aturan-kebijakan.destroy', $record->id) }}" method="POST"
