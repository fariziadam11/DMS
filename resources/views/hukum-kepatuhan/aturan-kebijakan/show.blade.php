...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Aturan Kebijakan</h1>
    </div>
    <div class="card">
        <div class="card-body">
                ...
                @if ($permissions['download'])
                    <tr>
                        ...
                        <td>
                            @if ($record->file_name)
                                <button
                                    onclick="previewFile('{{ route('hukum-kepatuhan.aturan-kebijakan.preview', $record->id) }}', '{{ $record->file_name }}')"
                                    class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i> Preview</button> <a
                                    href="{{ route('hukum-kepatuhan.aturan-kebijakan.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                <a href="{{ route('hukum-kepatuhan.aturan-kebijakan.index') }}" class="btn btn-outline-secondary"><i
                        class="bi bi-arrow-left"></i> Kembali</a>
                <div>
                    <a href="{{ route('hukum-kepatuhan.aturan-kebijakan.edit', $record->id) }}" class="btn btn-warning"><i
                            class="bi bi-pencil"></i> Edit</a>
                    <form action="{{ route('hukum-kepatuhan.aturan-kebijakan.destroy', $record->id) }}" method="POST"
