    ...
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        @if ($permissions['create'])
            <div class="d-flex gap-2">
                <a href="{{ route('keuangan.cashflow.import') }}" class="btn btn-success"><i
                        class="bi bi-file-earmark-excel me-1"></i> Import Excel</a>
                <a href="{{ route('keuangan.cashflow.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>
                    Tambah</a>
            </div>
        @endif
    </div>
                                    ...
                                    @if ($item->file_name)
                                        @if ($item->userHasFileAccess(auth()->id()))
                                            <div class="btn-group btn-group-sm">
                                                <button
                                                    onclick="previewFile('{{ route('keuangan.cashflow.preview', $item->id) }}', '{{ $item->file_name }}')"
                                                    class="btn btn-outline-primary" title="Preview"><i
                                                        class="bi bi-eye"></i></button>
                                                <a href="{{ route('keuangan.cashflow.download', $item->id) }}"
