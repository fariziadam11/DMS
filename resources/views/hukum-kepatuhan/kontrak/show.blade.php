...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Kontrak</h1>
    </div>
    <div class="card">
        <div class="card-body">
            <table class="table table-bordered">
                ...
                <tr>
                    <th width="200">Mulai</th>
                    <td>{{ $record->berlaku_awal ? date('d F Y', strtotime($record->berlaku_awal)) : '-' }}</td>
                </tr>
                <tr>
                    <th width="200">Akhir</th>
                    <td>{{ $record->berlaku_akhir ? date('d F Y', strtotime($record->berlaku_akhir)) : '-' }}</td>
                </tr>
                <tr>
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if($permissions['preview'] ?? false)
                                            <button
                                        onclick="previewFile('{{ route('hukum-kepatuhan.kontrak.preview', $record->id) }}', '{{ $record->file_name }}')"
                                        class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i> Preview</button>
                                            @endif
                                    <a href="{{ route('hukum-kepatuhan.kontrak.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                ...
                @else
                    <a href="{{ route('hukum-kepatuhan.kontrak.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('hukum-kepatuhan.kontrak.edit', $record->id) }}" class="btn btn-warning"><i
                                class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('hukum-kepatuhan.kontrak.destroy', $record->id) }}" method="POST"
