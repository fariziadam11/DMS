...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Department</h1>
    </div>
    <div class="card">
        <div class="card-body">
            <table class="table table-bordered">
                <tr>
                    <th width="200">Kode Department</th>
                    <td><span class="badge bg-info">{{ $record->kode_department }}</span></td>
                </tr>
                <tr>
                    <th>Nama Department</th>
                    <td><strong>{{ $record->nama_department }}</strong></td>
                </tr>
            ...
            <div class="d-flex justify-content-between">
                ...
                @elseif (request('source') == 'search')
                    <a href="{{ route('search') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left"></i> Kembali
                        ke Pencarian</a>

                @else
                    <a href="{{ route('master.department.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('master.department.edit', $record->id) }}" class="btn btn-warning"><i
                                class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('master.department.destroy', $record->id) }}" method="POST" class="d-inline">
