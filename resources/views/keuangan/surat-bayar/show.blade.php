...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Surat Bayar</h1>
    </div>
    <div class="card">
        <div class="card-body">
            <table class="table table-bordered">
                <tr>
                    ...
                    <td>{{ $record->tanggal_surat_bayar ? date('d F Y', strtotime($record->tanggal_surat_bayar)) : '-' }}
                    </td>
                </tr>
                <tr>
                    <th width="200">Nomor</th>
                    <td>{{ $record->nomor_surat_bayar ?? '-' }}</td>
                </tr>
                <tr>
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if ($permissions['preview'] ?? false)
                                        <button
                                            onclick="previewFile('{{ route('keuangan.surat-bayar.preview', $record->id) }}', '{{ $record->file_name }}')"
                                            class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i>
                                            Preview</button>
                                    @endif
                                    <a href="{{ route('keuangan.surat-bayar.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                ...
                @else
                    <a href="{{ route('keuangan.surat-bayar.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('keuangan.surat-bayar.edit', $record->id) }}" class="btn btn-warning"><i
                                class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('keuangan.surat-bayar.destroy', $record->id) }}" method="POST"
