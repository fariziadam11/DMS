...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail SPPB</h1>
    </div>
    <div class="card">
        <div class="card-body">
            <table class="table table-bordered">
                <tr>
                    ...
                    <td>{{ $record->tanggal_sppb ? date('d F Y', strtotime($record->tanggal_sppb)) : '-' }}</td>
                </tr>
                <tr>
                    <th width="200">Nomor</th>
                    <td>{{ $record->nomor_sppb ?? '-' }}</td>
                </tr>
                <tr>
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if ($permissions['preview'] ?? false)
                                        <button
                                            onclick="previewFile('{{ route('keuangan.sppb.preview', $record->id) }}', '{{ $record->file_name }}')"
                                            class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i>
                                            Preview</button>
                                    @endif
                                    <a href="{{ route('keuangan.sppb.download', $record->id) }}" class="btn btn-success"
            ...
            <div class="d-flex justify-content-between">
                ...
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('keuangan.sppb.edit', $record->id) }}" class="btn btn-warning"><i
                                class="bi bi-pencil"></i> Edit</a>
                    @endif
                    <form action="{{ route('keuangan.sppb.destroy', $record->id) }}" method="POST" class="d-inline">@csrf
