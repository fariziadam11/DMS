    ...
    <div class="card">
        <div class="card-body">
            <form
                ...
                @csrf @if (isset($record))
                    @method('PUT')
                @endif
                <div class="row g-4">
                    <div class="col-md-6"><label class="form-label">Divisi <span class="text-danger">*</span></label><select
                            name="id_divisi" class="form-select" required>
                            <option value="">Pilih Divisi</option>
                            @foreach ($divisions as $d)
                                <option value="{{ $d->id }}"
                                    {{ old('id_divisi', $record->id_divisi ?? '') == $d->id ? 'selected' : '' }}>
                                    {{ $d->nama_divisi }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6"><label class="form-label">Klasifikasi</label><select name="sifat_dokumen"
                            class="form-select">
                            <option value="Umum"
                    ...
                    <div class="col-md-6"><label class="form-label">Tanggal</label><input type="date" name="tanggal"
                            class="form-control"
                            value="{{ old('tanggal', isset($record) ? \Carbon\Carbon::parse($record->tanggal ?? ($record->tgl_surat ?? ($record->tgl_transaksi ?? now())))->format('Y-m-d') : '') }}">
                    </div>
                    ...
                    <div class="col-12"><label class="form-label">File @if (!isset($record))
                        ...
                        @if (isset($record) && $record->file)
                            <div class="mt-2"><span class="badge bg-success">{{ $record->file_name }}</span></div>
                        @endif
                    </div>
                    @if (isset($record))
                        <div class="col-12"><label class="form-label">Catatan Perubahan</label>
                            <textarea name="change_notes" class="form-control" rows="2"></textarea>
                        </div>
                    @endif
                </div>
                <hr class="my-4">
                <div class="d-flex justify-content-between"><a href="{{ route($routePrefix . '.index') }}"
                        class="btn btn-outline-secondary"><i class="bi bi-arrow-left"></i> Kembali</a><button type="submit"
                        class="btn btn-primary"><i class="bi bi-check-lg"></i>
                        {{ isset($record) ? 'Update' : 'Simpan' }}</button></div>
            </form>
        </div>
    </div>
@endsection
