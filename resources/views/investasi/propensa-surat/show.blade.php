...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Propensa Surat</h1>
    </div>
    <div class="card">
        <div class="card-body">
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if ($permissions['preview'] ?? false)
                                        <button
                                            onclick="previewFile('{{ route('investasi.propensa-surat.preview', $record->id) }}', '{{ $record->file_name }}')"
                                            class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i>
                                            Preview</button>
                                    @endif
                                    <a href="{{ route('investasi.propensa-surat.download', $record->id) }}"
