...
@section('content')
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('hukum-kepatuhan.aturan-kebijakan.create') }}" class="btn btn-primary"><i
                ...
                <tbody>
                    @forelse($items as $i => $item)
                        <tr>
                                    ...
                                    @if ($item->file_name)
                                        @if ($item->userHasFileAccess(auth()->id()))
                                            <div class="btn-group btn-group-sm">
                                                <button
                                                    onclick="previewFile('{{ route('hukum-kepatuhan.aturan-kebijakan.preview', $item->id) }}', '{{ $item->file_name }}')"
                                                    class="btn btn-outline-primary" title="Preview"><i
                                                        class="bi bi-eye"></i></button>
                                                <a href="{{ route('hukum-kepatuhan.aturan-kebijakan.download', $item->id) }}"
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('hukum-kepatuhan.aturan-kebijakan.show', $item->id) }}"
                                        class="btn btn-outline-primary"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('hukum-kepatuhan.aturan-kebijakan.edit', $item->id) }}"
                                        class="btn btn-outline-warning"><i class="bi bi-pencil"></i></a>
                                    <form action="{{ route('hukum-kepatuhan.aturan-kebijakan.destroy', $item->id) }}"
