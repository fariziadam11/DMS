...
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail Dokumen RRA</h1>
    </div>
    <div class="card">
        <div class="card-body">
            <table class="table table-bordered">
                ...
                <tr>
                    <th width="200">Nomor Nota</th>
                    <td>{{ $record->nomor_nota ?? '-' }}</td>
                </tr>
                <tr>
                    <th width="200">Nomor Sukka</th>
                    <td>{{ $record->nomor_sukka ?? '-' }}</td>
                </tr>
                <tr>
                            ...
                            @if ($record->file_name)
                                <div class="btn-group btn-group-sm">
                                    @if ($permissions['preview'] ?? false)
                                        <button
                                            onclick="previewFile('{{ route('anggaran.dokumen-rra.preview', $record->id) }}', '{{ $record->file_name }}')"
                                            class="btn btn-primary" title="Preview"><i class="bi bi-eye"></i>
                                            Preview</button>
                                    @endif
                                    <a href="{{ route('anggaran.dokumen-rra.download', $record->id) }}"
            ...
            <div class="d-flex justify-content-between">
                ...
                @else
                    <a href="{{ route('anggaran.dokumen-rra.index') }}" class="btn btn-outline-secondary"><i
                            class="bi bi-arrow-left"></i> Kembali</a>
                @endif
                <div>
                    @if ($permissions['edit'])
                        <a href="{{ route('anggaran.dokumen-rra.edit', $record->id) }}" class="btn btn-warning"><i
                                class="bi bi-pencil"></i> Edit</a>
                    @endif
                    @if ($permissions['delete'])
                        <form action="{{ route('anggaran.dokumen-rra.destroy', $record->id) }}" method="POST"
